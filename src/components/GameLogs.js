...
const GameLogs = ({ playerId, player: { stats: { 0: { splits: allSeasons } } }, swipeReferences, lastSeason: { season } }) => {
  ...
  const createFilters = () => {
    ...
    allSeasons.forEach(season => {
      if (season.league.name === "National Hockey League") {
        const seasonWithDash = season.season.slice(0, 4) + "-" + season.season.slice(4);
  ...
  const makeRows = (splits) => {
    ...
    splits.forEach(season => {
      const isAwayOrHome = season.isHome ? season.opponent.abbreviation : season.opponent.abbreviation.replace(/^/, '@');
      const a = createData(season.date, isAwayOrHome, season.stat.points, season.stat.goals, season.stat.assists, season.stat.timeOnIce);
      rows.push(a);
    });
    setGames(rows);
  ...
  useEffect(() => {
    if (typy(response, 'copyright').safeObject && playoffSelected === false) {
      const { stats: { 0: { splits } } } = response;
      makeRows(splits);
    } else if (typy(playoffResponse, 'copyright').safeObject && playoffSelected === true) {
  ...
  const dataFilter = filter => {
    ...
    // This causes new network request with spesific season
